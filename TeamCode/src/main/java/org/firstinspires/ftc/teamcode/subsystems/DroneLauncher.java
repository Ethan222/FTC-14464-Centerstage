...
public class DroneLauncher {
    ...
    public String getLauncherTelemetry() {
        return String.format(Locale.ENGLISH, "%s (%.0f%%) [%.2f]", getLauncherState(), launcher.getPercent(), launcher.getPosition());
    }
    public String getRotatorTelemetry() {
        return String.format(Locale.ENGLISH, "%s (%.0f%%) [%.2f]", getRotationState(), getRotationPercent(), getRotation());
    }
}
